					...
					if ( DOES_CHAR_EXIST(zombies[i]) )
					{
						...
						if ( (dist > 40.0) || IS_CHAR_DEAD(zombies[i]) || IS_CHAR_IN_WATER(zombies[i]) || IS_CHAR_IN_AIR(zombies[i])) 
						{
							...
							else
							{
								...
								if ((del_ped == 5) || (del_ped == 10) || (del_ped == 15) || (IS_CHAR_IN_AIR(zombies[i])))
								{
									SET_CHAR_COORDINATES_NO_OFFSET(zombies[i], 25.0, 25.0, 4.5);
									EXPLODE_CHAR_HEAD(zombies[i]);
									TASK_DIE(zombies[i]);
									MARK_CHAR_AS_NO_LONGER_NEEDED(&zombies[i]);
									//PRINT_STRING_WITH_LITERAL_STRING_NOW("string", "gen new ped", 1000, 1);
								}
								else
								{
									//рендом задержки тут
